...
const getCTAOption = async (req, res) => {
    ...
    try {
        if (email) {
            const { data, error } = await DB.from(DB_User).select(`cta_option`).eq('email', email)
...
const addCTAOption = async (req, res) => {
    const { email } = req.user
    const AllCTA = await DB.from(DB_User).select(`cta_option`).eq("email", email)
    ...
    const update = await DB.from(DB_User).update({ cta_option: NewDataCTA }).eq("email", email)
